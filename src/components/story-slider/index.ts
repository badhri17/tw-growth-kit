 ...
 *   • Per-slide product picker → CTA falls back to the product URL.
 */
export default class GrowthStorySlider extends LitElement {
  ...
  private _slides(): StorySlideItem[] {
    ...
    return list.filter((s) => {
      ...
      return !!(s.image || s.image_desktop || s.video || s.product);
    });
  }

  // ------------------------------------------------------------
  // Product resolution (CTA fallback when slide has no cta_url)
  // ------------------------------------------------------------
  ...
  private async _fetchProduct(id: number, label: string) {
    ...
    try {
      ...
      console.warn("[growth-story-slider] product fetch failed", id, err);
      this._productCache.set(id, { status: "failed" });
    }
    this.requestUpdate();
  }

  private _resolveProduct(slide: StorySlideItem): ResolvedProduct | null {
        ...
        >
          ...
          <div class="ss-track">
            ${slides.map((slide, i) => {
              ...
              const baseSrc = slide.image || product?.image || "";
              const desktopSrc = slide.image_desktop || baseSrc;
              const title = this._t(slide.title) || product?.name || "";
              ...
              const ctaLabel =
                this._t(slide.cta_label) ||
                (slide.cta_url || product ? defaultCtaLabel : "");
              const ctaHref = this._slideCtaHref(slide, product);
  ...
  private _renderPagination(
    ...
    if (style === "fraction") {
      // The signature: "02 / 04" with a thin elegant divider track.
        ...
        <div class="ss-pagination ss-pag-fraction" role="status" aria-live="polite">
          ...
          ${showTrack
            ? html`<span
                ...
                aria-hidden="true"
              ></span>`
            : html`<span class="ss-pag-sep" aria-hidden="true">${separator}</span>`}
