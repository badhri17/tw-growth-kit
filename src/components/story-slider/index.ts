...
export default class GrowthStorySlider extends GrowthElement {
  ...
  private _onPointerDown = (e: PointerEvent) => {
    ...
    // retarget the eventual `click` to the frame (nearest common ancestor of
  ...
  private _onPointerMove = (e: PointerEvent) => {
    ...
    // the frame before a swipe started (nothing was captured yet) — drop the
    ...
    if (!this._swipeActive && Math.abs(dx) > 10 && Math.abs(dx) > Math.abs(dy)) {
      ...
      // Capture now so a release outside the frame still lands on our
