...
2. **Each duplicated copy MUST have a UNIQUE `id` — distinct `key`s are NOT enough.** Salla's admin form builder keys field state and condition gating by `id`. When 2+ conditional fields share one `id`, gating **silently breaks**: the field renders **zero times** for one selected value and **N times** (all copies stacked) for another. (Statics with unique ids in the same group keep working — that's the tell.) So name copies `bg_effect`, `bg_effect_floating`, `bg_effect_split`, … and resolve the active one in the component by reading the controlling value (`layout === "floating" ? c.bg_effect_floating : …`). Canonical examples: featured-product (`bg_effect*`, `aspect_ratio*`, `image_fit*`).
